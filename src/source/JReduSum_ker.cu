...
//==============================================================================
double ReduSumDoubleBase(unsigned ndata,unsigned inidata,const double* data,double* resu,double *pim1_sum,hipStream_t stm){
  double ret=0;
  if(ndata){
    ...
    double *res=resu1;
    while(n>1){
      hipLaunchKernelGGL(HIP_KERNEL_NAME(KerReduSumDouble<REDUBSIZE>), sgrid, REDUBSIZE, smemSize, stm, n,ini,dat,res);
      n=nblocks; ini=0;
...
//==============================================================================
float ReduSumFloatBase(unsigned ndata,unsigned inidata,const float* data,float* resu,float *pim1_sum,hipStream_t stm){
  float ret=0;
  if(ndata){
    unsigned n=ndata,ini=inidata;
    ...
    float *res=resu1;
    while(n>1){
      hipLaunchKernelGGL(HIP_KERNEL_NAME(KerReduSumFloat<REDUBSIZE>), sgrid, REDUBSIZE, smemSize, stm, n,ini,dat,res);
      n=nblocks; ini=0;
      sgrid=GetSimpleGridSize(n,REDUBSIZE);  
      nblocks=sgrid.x*sgrid.y;
      if(n>1){
...
//==============================================================================
unsigned ReduSumUintBase(unsigned ndata,unsigned inidata,const unsigned* data,unsigned* resu,unsigned *pim1_sum,hipStream_t stm){
  unsigned ret=0;
  if(ndata){
    unsigned n=ndata,ini=inidata;
    ...
    unsigned *res=resu1;
    while(n>1){
      hipLaunchKernelGGL(HIP_KERNEL_NAME(KerReduSumUint<REDUBSIZE>), sgrid, REDUBSIZE, smemSize, stm, n,ini,dat,res);
      n=nblocks; ini=0;
      sgrid=GetSimpleGridSize(n,REDUBSIZE);  
      nblocks=sgrid.x*sgrid.y;
      if(n>1){
...
/// Usar ReduSumFloat() o ReduSumFloatAsyn().
//==============================================================================
float3 ReduSumFloat3Base(unsigned ndata,unsigned inidata,const float3* data,float3* resu,float3 *pim1_sum,hipStream_t stm){
  ...
  if(ndata){
    ...
    float3 *res=resu1;
    while(n>1){
      hipLaunchKernelGGL(HIP_KERNEL_NAME(KerReduSumFloat3<REDUBSIZE>), sgrid, REDUBSIZE, smemSize, stm, n,ini,dat,res);
      n=nblocks; ini=0;
      sgrid=GetSimpleGridSize(n,REDUBSIZE);  
      nblocks=sgrid.x*sgrid.y;
      if(n>1){
