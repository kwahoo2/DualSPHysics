...
3. Edit the `Makefile` file with a text editor and then:
   * Set the `DIRTOOLKIT` variable with the path to ROCm in your system e.g. `DIRTOOLKIT=/opt/rocm-4.5.0`
   * Adjust the `CCFLAGS` variable with info got from the `hipconfig --cxx_config` command, e.g. `CCFLAGS +=-D__HIP_PLATFORM_HCC__= -D__HIP_PLATFORM_AMD__= -I/opt/rocm-4.5.0/hip/include -I/opt/rocm-4.5.0/llvm/bin/../lib/clang/13.0.0 -I/opt/rocm-4.5.0/hsa/include`
